...
public class Percolation {
	...
	public Percolation(int n) {
		...
		for (int j = 1; j <= n; j++) {
			// add all upper fields to be connected to source
			this.percolateFind.union(this.source, calculateId(1, j));
			this.fullFind.union(this.source, calculateId(1, j));
			
			// add all bottom fields to be connected to sink
			this.percolateFind.union(calculateId(n, j), this.sink);
		}
	...
	public boolean isFull(int i, int j) {
		checkBounds(i, j);
		return isOpen(i, j) 
				&& this.fullFind.connected(this.source, calculateId(i, j));
	}
	...
	private boolean canConnect(int neighbourI, int neighbourJ) {
		return neighbourI >= 1 && neighbourI <= this.open.length 
				&&  neighbourJ >= 1 && neighbourJ <= this.open.length 
